...
export class MeasureSyncHealthJobScheduler {
  ...
  private _startSecondsAgo = 60 * 10;
  private _spanSeconds = 60 * 5;
  ...
  async doJobs() {
    ...
    for (const peerInfo of peersToContact) {
      const rpcClient = await this._hub.getRPCClientForPeer(peerInfo.peerId, peerInfo.contactInfo);

      if (rpcClient === undefined) {
        log.info("Couldn't get rpc client, skipping peer", peerInfo);
        continue;
      }
